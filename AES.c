...
//Rotates 32 bit word left by n bits
uint32_t rotate (uint32_t x, int n)
{
	return (x << n) | (x >> (32 - n));
}

//selects a given byte (0-3) from a 32 bit word and returns it. 
uint8_t get_byte(uint32_t word, int byte)
{
	uint32_t mask = 0xFF000000;
	mask >>= (byte * 8);
	return (word & mask) >> (24 - (byte * 8));
}

//sets a given byte (0-3) in a 32 bit word to the input byte
uint32_t set_byte(uint32_t word, uint8_t byte, int loc)
{
 	uint32_t mask = 0xFF000000;
 	mask >>= (loc * 8);
 	mask = ~mask; //invert mask because we want to zero an area
 	word = word & mask;
 	return word | (byte << (24 - (loc * 8)));
}
